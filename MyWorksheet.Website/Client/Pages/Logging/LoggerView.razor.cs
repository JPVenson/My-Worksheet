...
public partial class LoggerView
{
    ...
    public RingBuffer<AppLoggerLogViewModel> LoggerEntries { get; set; }

    public override async Task LoadDataAsync()
    {
        await base.LoadDataAsync();
        var logs = ServerErrorManager.EvalAndUnbox(await HttpService.LoggerApiAccess.GetEntries(null, 1, 50));
        LoggerEntries = new RingBuffer<AppLoggerLogViewModel>(200, true);
        foreach (var appLoggerLogViewModel in logs?.Reverse() ?? Enumerable.Empty<AppLoggerLogViewModel>())
        {
            LoggerEntries.Add(appLoggerLogViewModel);
        }
        AddAsyncDisposable(await SignalService.LoggerEntryHub.BeginLogging(NewLogEntries));
    }

    private async Task NewLogEntries(AppLoggerLogViewModel[] obj)
    {
        await InvokeAsync(() =>
        {
            foreach (var loggerEntryViewModel in obj)
            {
                LoggerEntries.Add(loggerEntryViewModel);
            }
